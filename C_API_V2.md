...
- **Vector reads are a single untyped view.** PR4 ships one `duckdb_v2_vector_view` struct (`{const void *data; const uint64_t *validity; const duckdb_v2_sel_t *sel}` — pointer triple, no count) and one `duckdb_v2_vector_get_view(vec, &view, err)` getter for every kind. The row count is the caller's context, not the view's: it comes from `data_chunk_get_size` for top-level vectors and from parent geometry for nested children (`vector_list_get_size` for LIST/MAP children; `parent_count * array_size` for ARRAY; parent count for STRUCT/UNION children). Callers dispatch on `LogicalTypeId` and cast `view.data` to the matching typed pointer or layout typedef (`duckdb_v2_hugeint_t`, `duckdb_v2_interval_t`, `duckdb_v2_list_entry`, `duckdb_v2_varchar_t`/`blob_t`/`bit_t`/`bignum_t` — all aliases for `duckdb_v2_string_t`).

- **Structural descent uses two generic accessors.** `duckdb_v2_vector_get_child_count(vec, &n, err)` returns the number of children for a nested vector; `duckdb_v2_vector_get_child(vec, idx, &child, err)` borrows one. Per-kind index convention: LIST → `[0]` = elements; MAP → `[0]` = keys, `[1]` = values (V2 hides MAP's internal `LIST<STRUCT(K,V)>`); ARRAY → `[0]` = elements; STRUCT → `[i]` = field `i`; UNION → `[0]` = tag, `[1..N]` = members. "How many fields" / "how many members" are answered by PR1's `logical_type_get_struct_child_count` / `_get_union_member_count`, not by separate vector-side accessors. The one count getter on `Vector` is `duckdb_v2_vector_list_get_size` — LIST/MAP only, returns `ListVector::GetListSize`.
