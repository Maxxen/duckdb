...
// ---------------------------------------------------------------------------
// ---------------------------------------------------------------------------
// A client with only a connection builds, registers, and calls a scalar
// function. The builder is created and configured with no context in hand;
// register_on_connection runs the catalog insert in its own transaction on the
// connection's context.
// ---------------------------------------------------------------------------
TEST_CASE("V2 scalar: register_on_connection", "[capi_v2][scalar]") {
	...
	static auto exec_callback = [](duckdb_v2_scalar_function_exec_info_handle info, duckdb_v2_context_handle,
	...
	// Build entirely outside any context.
	...
	REQUIRE(duckdb_v2_scalar_function_builder_set_name(builder, V2Str("add_outside"), nullptr) == DUCKDB_V2_ERROR_NONE);
	...
	REQUIRE(duckdb_v2_scalar_function_builder_set_exec_callback(builder, exec_callback, nullptr) ==
	        DUCKDB_V2_ERROR_NONE);

	// Register straight from the connection.
	REQUIRE(duckdb_v2_scalar_function_builder_register_with_connection(conn, builder, nullptr) == DUCKDB_V2_ERROR_NONE);
	// A null connection is rejected.
	REQUIRE(duckdb_v2_scalar_function_builder_register_with_connection(nullptr, builder, nullptr) ==
	...
	REQUIRE(V2Query(conn, "SELECT add_outside(4, 5)", &result, nullptr) == DUCKDB_V2_ERROR_NONE);
	...
	REQUIRE((static_cast<const int32_t *>(result_view.data))[0] == 9);
...
TEST_CASE("V2 scalar: parameter defaults and named-argument calls", "[capi_v2][scalar][signature]") {
	...
	REQUIRE(duckdb_v2_scalar_function_builder_register_with_connection(fix.conn, b, nullptr) == DUCKDB_V2_ERROR_NONE);
