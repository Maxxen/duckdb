...
unique_ptr<FunctionData> BindReservoirQuantileDecimal(BindAggregateFunctionInput &input) {
	auto &function = input.GetBoundFunction();
	auto &arguments = input.GetArguments();
	function.ReplaceDefinition(GetReservoirQuantileAggregateFunction(arguments[0]->return_type.InternalType()));
...
void DefineReservoirQuantile(AggregateFunctionSet &set, const LogicalType &type) {
	...
	fun.GetSignature().AddParemeter("", LogicalType::INTEGER);
	set.AddFunction(fun);
