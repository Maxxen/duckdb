...
unique_ptr<FunctionData> BindApproxQuantileDecimal(BindAggregateFunctionInput &input) {
	...
	function.ReplaceDefinition(ApproxQuantileDecimalFunction(arguments[0]->return_type));
	return bind_data;
}
...
AggregateFunction GetApproximateQuantileAggregate(const LogicalType &type) {
	...
	fun.GetSignature().AddParemeter("", LogicalType::FLOAT);
	return fun;
}
...
unique_ptr<FunctionData> BindApproxQuantileDecimalList(BindAggregateFunctionInput &input) {
	...
	auto &arguments = input.GetArguments();
	auto bind_data = BindApproxQuantile(input);
	function.ReplaceDefinition(ApproxQuantileDecimalListFunction(arguments[0]->return_type));
	return bind_data;
}
...
AggregateFunction GetApproxQuantileListAggregate(const LogicalType &type) {
	...
	fun.GetSignature().AddParemeter("", list_of_float);
	return fun;
}
...
unique_ptr<FunctionData> ApproxQuantileDecimalDeserialize(Deserializer &deserializer,
	...
	if (return_type.id() == LogicalTypeId::LIST) {
		function.ReplaceDefinition(ApproxQuantileDecimalListFunction(function.arguments[0]));
	} else {
		function.ReplaceDefinition(ApproxQuantileDecimalFunction(function.arguments[0]));
	}
	return bind_data;
}
