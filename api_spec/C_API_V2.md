...
The **C API V2** (`duckdb_v2.h`) is a declaratively-defined C interface to DuckDB. Its public header and the bridge implementations that back it are generated from YAML specifications under `api_spec/v2/`. It coexists with the original (V1) C API (`duckdb.h`), which is unaffected: every V2 identifier is `duckdb_v2_` / `DUCKDB_V2_` prefixed, so the two never collide and either may be used.

This document is the V2 design reference: the contracts and the reasoning behind them (the error model, the streaming result model, replacement scans, the stable C++ API and its data model, and the load-bearing conventions). Where a contract records an alternative that was considered and rejected, that reasoning is part of it: it explains why the surface is shaped the way it is.

V2 is:

- **Prefixed** — all identifiers use `duckdb_v2_` / `DUCKDB_V2_`, so there are no symbol collisions with the original C API.
- **Declarative** — the API surface is defined in YAML under `api_spec/v2/`. The public C header (`src/include/duckdb_v2.h`) and the per-function stub skeleton (`src/main/capi_v2/capi_v2_stubs.cpp`) are generated from those specs by capigen. Real implementations are hand-written next to the stubs; the generator drops a stub once it finds a matching hand-written definition.
- **Validated** through Catch2 bridge tests under `test/api/capi_v2/`.

## Related documentation

- **Building, setup, and regeneration** (provisioning the toolchain, the pre-commit pipeline, regenerating the header and stubs, running the tests, and CI): the "C API V2 development" section of `CONTRIBUTING.md`.
- **capigen** (github.com/duckdb/capigen): the generator that produces the C header and bridge stubs from the specs. Its `schema_reference.md` is the IDL field reference and its `CLAUDE.md` documents the spec-language features.
...
api_spec/                        API specs (YAML) -- the canonical API definitions
  C_API_V2.md                    This design reference
  v2/                            The V2 API
...
| layout | committed byte mirror (cast target) | `NameLayout` | `HugeintLayout`, `IntervalLayout`, `BytesLayout` |
...
`IntervalLayout`, `HugeintLayout`, `UhugeintLayout`, `BytesLayout`, used by both
...
the decoded wire bytes (VARCHAR content), a vector slot is the `BytesLayout` string_t.
...
These rules apply when writing V2 spec YAML, bridge implementations, and tests.
...
- **Three registration scopes: connection, extension, context.** Catalog entries (scalar / aggregate / table / copy / cast functions, custom types) and database-level hooks (replacement scans, log storages) are installed either from *outside* — `_register_with_connection` for catalog entries, `_register_with_database` for database-level hooks, each running the install in its own transaction — or from an *extension load*, via `_register_with_extension` on the borrowed `extension` handle the library hands out for the duration of the load, which installs on the database's system transaction and attributes the entry to the extension. A `context` is deliberately **not** a registration scope: a context is handed out inside arbitrary callbacks (a scalar function's exec, a replacement scan), where installing a new catalog entry makes no sense. Contexts remain the scope for *reading* and for constructing values and types (`logical_type_create_from_args`, `value_cast_with_context`, …), and an extension-load callback carries both handles so a load can do either. Builders themselves are context-free: construct and configure them anywhere, then register with whichever scope is in hand.

- **Handle layout is load-bearing.** A V2 handle is a raw pointer to the underlying C++ object — *not* a wrapper struct — unless the wrapper is documented as load-bearing (`EnvironmentWrapperV2`, `OptionWrapperV2`, etc.). Handles are emitted as `tagged_struct` typedefs (`struct _duckdb_v2_x { void *internal_ptr; } * duckdb_v2_x_handle`), but that struct is a compile-time type tag only: the bridge `reinterpret_cast`s the C++ object pointer straight to it, so the runtime identity is still the bare object pointer. `duckdb_v2_logical_type_handle` specifically is a `duckdb::LogicalType *`; V1 and V2 share the same `new LogicalType(...)` allocation, so V2 destroy can free a V1-built handle. Direction matters: V1 → V2 destroy is OK; V2 → V1 destroy is not asserted and must not be relied on. **Do not wrap `duckdb_v2_logical_type` in a struct** — the identity is relied on by the deliberate V1-interop pins: the V1-oracle and the two cross-version round-trip tests in `test_capi_v2_logical_type.cpp`, the V1 `string_t` cross-validation in `test_capi_v2_bytes_layout.cpp`, the `[v1_v2_bridge]` value test, and the V1-only zero-entry-enum pins (SQL forbids the type; V2 inspection degrades gracefully). Same rule for `duckdb_v2_data_chunk_handle` (a `duckdb::DataChunk *`) and `duckdb_v2_vector_handle` (a `duckdb::Vector *`): no wrappers. Neither needs to carry per-handle state — cardinality flows through the API explicitly, and the single untyped view-getter is thin enough to extract `(data, validity, sel)` directly from the core helpers without caching a `UnifiedVectorFormat`.
...
- **Opaque user data crosses as one struct — `duckdb_v2_opaque`.** Any caller-owned pointer the engine must hold (user data, bind data, init / global / local state) crosses the ABI as the single by-value `duckdb_v2_opaque {void *ptr; …destroy; …equals;}`, never a bespoke `(void *user_data, user_data_destroy)` pair, and there is no `user_data_copy` callback. In the bridge, hold it in the `OpaqueDataHandle` RAII wrapper (`capi_v2_internal.hpp`): `shared_ptr<OpaqueDataHandle>` when the data is threaded / copied across phases (bind data, builder user data), a plain `OpaqueDataHandle` by value for single-owner state. The stable C++ API builds one via `detail::MakeUserData<T>` (raw fn pointer + this channel). New user-data-taking surface must use this, not reinvent it.
...
- **V1/V2 header separation.** Public V2 headers stay V1-free (`duckdb_v2.h` does not include `duckdb.h`). V2 `.cpp` implementations are free to wrap V1 or internal C++ machinery — the V2 contract is the shipped surface, not the implementation. **Tests are the only place V1 and V2 C headers may co-exist**, and only as deliberate interop validation pins. Fixture convenience is not a sanctioned reason: test fixtures use the V2 construction paths.
...
- **Vector reads are a single untyped view.** V2 exposes one `duckdb_v2_vector_view` struct (`{const void *data; const uint64_t *validity; const duckdb_v2_sel_t *sel, idx_t count; }`) and one `duckdb_v2_vector_get_view(vec, &view, err)` getter for every kind. Callers dispatch on `LogicalTypeId` and cast `view.data` to the matching typed pointer or layout typedef (`duckdb_v2_hugeint_t`, `duckdb_v2_interval_t`, `duckdb_v2_list_entry`, `duckdb_v2_varchar_t`/`blob_t`/`bit_t`/`bignum_t`, all aliases for `duckdb_v2_bytes`).
...
- **Length-delimited strings (`duckdb_v2_str`).** Every borrowed string crossing the boundary, both inputs (option/SQL/function names, file paths, log messages) and outputs (option getters, `schema_get_field` (the field name), `logical_type_get_name`, `logical_type_get_param` (the name), `expression_get_function_name`, `error_info_get_text`), is a `duckdb_v2_str { const char *ptr; idx_t len; }`. It is a *borrowed view*: NOT null-terminated, may contain interior NULs, and `{NULL, 0}` is the canonical empty view (`ptr` must not be dereferenced when `len == 0`). Lifetime is documented per producing function (typically "valid until the owning handle is destroyed"). Validation contract for inputs: `{NULL, 0}` is a valid empty string; only a null pointer with a *nonzero* length is malformed and rejected with `ERROR_INVALID_INPUT`. Do not confuse `duckdb_v2_str` (the decoded view) with `duckdb_v2_bytes` (the 16-byte VARCHAR/BLOB/BIT/BIGNUM *storage*, a transparent union mirroring `duckdb::string_t`). Owned string returns that the caller must `free()` (e.g. `value_to_string`, `library_version`) keep their `char **` shape and are not `duckdb_v2_str`.

- **String-backed kinds: transparent storage, decoders only for real wire codecs.** The storage type `duckdb_v2_bytes` is a **transparent** union mirroring `duckdb::string_t` (`value.pointer.{length,prefix,ptr}` / `value.inlined.{length,inlined}`, inline cutoff `DUCKDB_V2_BYTES_INLINE_LENGTH`), spec-declared and capigen-emitted: callers read its fields directly and stride `arr[row]`; `static_assert`s in `capi_v2_internal.hpp` guard the layout match. VARCHAR / BLOB carry no wire encoding, so reading them is that direct field read and there is no varchar/blob decoder (one would be a redundant field read, failing the primitives test). BIT and BIGNUM do carry one and keep their bridge decoders: `duckdb_v2_bit_decode` peels the padding byte, `duckdb_v2_bignum_decode` produces an owned magnitude buffer from the bit-inverted negative storage (sharing `DecodeBignumStringT` with `value_get_bignum`). Per-kind read conveniences live in the stable C++ API.

- **Writing string-backed values is decomposed: get-arena, allocate, assemble, place.** There is no per-value `assign_string` and no copy-in bridge. Borrow the vector's arena once with `duckdb_v2_vector_get_arena` (the single string-ness check; valid for the physical-VARCHAR kinds VARCHAR / BLOB / BIT / BIGNUM), reserve vector-lifetime bytes with `duckdb_v2_arena_allocate` (raw bump allocation: any `byte_len` incl. 0, no value semantics, no gating), write them, assemble a `duckdb_v2_bytes` over the transparent layout, and place it through `duckdb_v2_vector_get_data_mutable`. Values fitting `DUCKDB_V2_BYTES_INLINE_LENGTH` need no allocation. The raw C layer is deliberately *only* arena allocation: now that `duckdb_v2_bytes` is transparent, assembling the value is plain layout work the caller (or the C++ layer) does without crossing the boundary, so a copy-in bridge would be redundant. Lifetime: allocated bytes, and any non-inlined `duckdb_v2_bytes` referencing them, are valid only in a slot of the owning vector; inlined values are self-contained. Ergonomics live in the C++ API: `Vector::AssignString` / `AssignStrings` for in-order fills, and `Vector::GetStringHeap` -> `StringHeap::Allocate` / `Add` / `AddMany` -> `Vector::SetString` for dedup / scatter / write-in-place, with `BytesLayout` mirroring `duckdb_v2_bytes`.
