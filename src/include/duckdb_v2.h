 ...
 *
 * The alias keeps the base type's internal representation, so the execution engine needs no custom handling for it,
 * while making the type logically distinct from its base. Intended for custom type bind callbacks, where the base type
 * and the name both come from the bind info.
 *
 ...
 *
 * The dual of value_create_uuid: the storage form a vector slot holds, with the high bit flipped for sort order, not
 * the canonical byte order. A value of any other type id, and a NULL value, return ERROR_INPUT_INVALID.
 *
 * history:
 * - stable: v2.0.0
 *
