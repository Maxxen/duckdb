...
// originally published under the CC0 Public Domain License,
// further modified by the DuckDB authors.
...
// mimic stl casing, allow implicit conversion
// NOLINTBEGIN(readability-identifier-naming,google-explicit-constructor)

#if (defined(_MSC_VER) && _MSC_VER == 1900)
#define DUCKDB_OPTIONAL_MSVC2015
#endif

#if (defined(__GNUC__) && __GNUC__ == 4 && __GNUC_MINOR__ <= 9 && !defined(__clang__))
#define DUCKDB_OPTIONAL_GCC49
#endif

#if (defined(__GNUC__) && __GNUC__ == 5 && __GNUC_MINOR__ <= 4 && !defined(__clang__))
#define DUCKDB_OPTIONAL_GCC54
#endif

#if (defined(__GNUC__) && __GNUC__ == 5 && __GNUC_MINOR__ <= 5 && !defined(__clang__))
#define DUCKDB_OPTIONAL_GCC55
#endif

#if (defined(__GNUC__) && __GNUC__ == 4 && __GNUC_MINOR__ <= 9 && !defined(__clang__))
// GCC < 5 doesn't support overloading on const&& for member functions
#define DUCKDB_OPTIONAL_NO_CONSTRR

// GCC < 5 doesn't support some standard C++11 type traits
#define DUCKDB_OPTIONAL_IS_TRIVIALLY_COPY_CONSTRUCTIBLE(T) std::has_trivial_copy_constructor<T>::value
#define DUCKDB_OPTIONAL_IS_TRIVIALLY_COPY_ASSIGNABLE(T)    std::has_trivial_copy_assign<T>::value

// This one will be different for GCC 5.7 if it's ever supported
#define DUCKDB_OPTIONAL_IS_TRIVIALLY_DESTRUCTIBLE(T) std::is_trivially_destructible<T>::value

// GCC 5 < v < 8 has a bug in is_trivially_copy_constructible which breaks std::vector
// for non-copyable types
#elif (defined(__GNUC__) && __GNUC__ < 8 && !defined(__clang__))
#ifndef DUCKDB_GCC_LESS_8_TRIVIALLY_COPY_CONSTRUCTIBLE_MUTEX
#define DUCKDB_GCC_LESS_8_TRIVIALLY_COPY_CONSTRUCTIBLE_MUTEX
namespace duckdb {
namespace detail {
template <class T>
...
#define DUCKDB_OPTIONAL_IS_TRIVIALLY_COPY_CONSTRUCTIBLE(T) tl::detail::is_trivially_copy_constructible<T>::value
#define DUCKDB_OPTIONAL_IS_TRIVIALLY_COPY_ASSIGNABLE(T)    std::is_trivially_copy_assignable<T>::value
#define DUCKDB_OPTIONAL_IS_TRIVIALLY_DESTRUCTIBLE(T)       std::is_trivially_destructible<T>::value
#else
#define DUCKDB_OPTIONAL_IS_TRIVIALLY_COPY_CONSTRUCTIBLE(T) std::is_trivially_copy_constructible<T>::value
#define DUCKDB_OPTIONAL_IS_TRIVIALLY_COPY_ASSIGNABLE(T)    std::is_trivially_copy_assignable<T>::value
#define DUCKDB_OPTIONAL_IS_TRIVIALLY_DESTRUCTIBLE(T)       std::is_trivially_destructible<T>::value
#endif

#if __cplusplus > 201103L
#define DUCKDB_OPTIONAL_CXX14
#endif

// constexpr implies const in C++11, not C++14
#if (__cplusplus == 201103L || defined(DUCKDB_OPTIONAL_MSVC2015) || defined(DUCKDB_OPTIONAL_GCC49))
#define DUCKDB_OPTIONAL_11_CONSTEXPR
#else
#define DUCKDB_OPTIONAL_11_CONSTEXPR constexpr
#endif

namespace duckdb {
#ifndef DUCKDB_MONOSTATE_INPLACE_MUTEX
#define DUCKDB_MONOSTATE_INPLACE_MUTEX
...
namespace detail {
#ifndef DUCKDB_TRAITS_MUTEX
#define DUCKDB_TRAITS_MUTEX
...
#define DUCKDB_TRAITS_LIBCXX_MEM_FN_WORKAROUND
#endif

// In C++11 mode, there's an issue in libc++'s std::mem_fn
// which results in a hard-error when using it in a noexcept expression
// in some cases. This is a check to workaround the common failing case.
#ifdef DUCKDB_TRAITS_LIBCXX_MEM_FN_WORKAROUND
template <class T>
...
struct optional_storage_base {
	DUCKDB_OPTIONAL_11_CONSTEXPR optional_storage_base() noexcept : m_dummy(), m_has_value(false) {
	}

	template <class... U>
	DUCKDB_OPTIONAL_11_CONSTEXPR optional_storage_base(in_place_t, U &&...u)
...
struct optional_operations_base : optional_storage_base<T> {
	...
	DUCKDB_OPTIONAL_11_CONSTEXPR T &get() & {
		return this->m_value;
	}
	DUCKDB_OPTIONAL_11_CONSTEXPR const T &get() const & {
		return this->m_value;
	}
	DUCKDB_OPTIONAL_11_CONSTEXPR T &&get() && {
		return std::move(this->m_value);
	}
#ifndef DUCKDB_OPTIONAL_NO_CONSTRR
...
template <class T, bool = DUCKDB_OPTIONAL_IS_TRIVIALLY_COPY_CONSTRUCTIBLE(T)>
...
template <class T, bool = DUCKDB_OPTIONAL_IS_TRIVIALLY_COPY_ASSIGNABLE(T) &&
                          DUCKDB_OPTIONAL_IS_TRIVIALLY_COPY_CONSTRUCTIBLE(T) &&
                          DUCKDB_OPTIONAL_IS_TRIVIALLY_DESTRUCTIBLE(T)>
...
// http://www.open-std.org/jtc1/sc22/wg21/docs/papers/2017/p0826r0.html
#if defined(DUCKDB_OPTIONAL_CXX14) && !defined(DUCKDB_OPTIONAL_GCC49) && !defined(DUCKDB_OPTIONAL_GCC54) &&            \
    !defined(DUCKDB_OPTIONAL_GCC55)
	/// Carries out some operation which returns an optional on the stored
	/// object if there is one.
	template <class F>
	DUCKDB_OPTIONAL_11_CONSTEXPR auto and_then(F &&f) & {
	...
	/// object if there is one.
	template <class F>
	DUCKDB_OPTIONAL_11_CONSTEXPR detail::invoke_result_t<F, T &> and_then(F &&f) & {
		using result = detail::invoke_result_t<F, T &>;
		static_assert(detail::is_optional<result>::value, "F must return an optional");
	...
	template <class F>
	DUCKDB_OPTIONAL_11_CONSTEXPR auto map(F &&f) & {
	...
	/// Carries out some operation on the stored object if there is one.
	template <class F>
	DUCKDB_OPTIONAL_11_CONSTEXPR decltype(optional_map_impl(std::declval<optional &>(), std::declval<F &&>()))
	map(F &&f) & {
		return optional_map_impl(*this, std::forward<F>(f));
	}
	...
	/// Carries out some operation on the stored object if there is one.
	template <class F>
	DUCKDB_OPTIONAL_11_CONSTEXPR auto transform(F &&f) & {
		return optional_map_impl(*this, std::forward<F>(f));
	}
	...
	optional<T> DUCKDB_OPTIONAL_11_CONSTEXPR or_else(F &&f) & {
		if (has_value()) {
			return *this;
		}
	...
	template <class F, detail::disable_if_ret_void<F> * = nullptr>
	optional<T> DUCKDB_OPTIONAL_11_CONSTEXPR or_else(F &&f) const & {
		return has_value() ? *this : std::forward<F>(f)();
	}
	...
	DUCKDB_OPTIONAL_11_CONSTEXPR optional disjunction(const optional &rhs) & {
	...
	DUCKDB_OPTIONAL_11_CONSTEXPR optional disjunction(optional &&rhs) & {
	...
	DUCKDB_OPTIONAL_11_CONSTEXPR optional(const optional &rhs) = default;

	/// Move constructor
	///
	/// If `rhs` contains a value, the stored value is direct-initialized with
	/// it. Otherwise, the constructed optional is empty.
	DUCKDB_OPTIONAL_11_CONSTEXPR optional(optional &&rhs) = default;
	...
	DUCKDB_OPTIONAL_11_CONSTEXPR explicit optional(
	...
	DUCKDB_OPTIONAL_11_CONSTEXPR T *operator->() {
		return std::addressof(this->m_value);
	}
	...
	DUCKDB_OPTIONAL_11_CONSTEXPR T &value() & {
		if (has_value()) {
			return this->m_value;
		}
		throw InternalException("Optional has no value");
	}
	DUCKDB_OPTIONAL_11_CONSTEXPR const T &value() const & {
		if (has_value()) {
			return this->m_value;
		}
		throw InternalException("Optional has no value");
	...
	template <class U>
	DUCKDB_OPTIONAL_11_CONSTEXPR T value_or(U &&u) && {
...
// http://www.open-std.org/jtc1/sc22/wg21/docs/papers/2017/p0826r0.html
	...
	/// Carries out some operation on the stored object if there is one.
	template <class F>
	DUCKDB_OPTIONAL_11_CONSTEXPR decltype(detail::optional_map_impl(std::declval<optional &>(), std::declval<F &&>()))
	map(F &&f) & {
		return detail::optional_map_impl(*this, std::forward<F>(f));
	}
	...
	/// If `rhs` contains a value, the stored value is direct-initialized with
	/// it. Otherwise, the constructed optional is empty.
	DUCKDB_OPTIONAL_11_CONSTEXPR optional(const optional &rhs) noexcept = default;
	...
	DUCKDB_OPTIONAL_11_CONSTEXPR T *operator->() noexcept {
		return m_value;
	}
	...
	/// Returns the contained value if there is one, otherwise throws InternalException
	DUCKDB_OPTIONAL_11_CONSTEXPR T &value() {
		if (has_value()) {
			return *m_value;
		}
	...
	/// \group value_or
	template <class U>
	DUCKDB_OPTIONAL_11_CONSTEXPR T value_or(U &&u) &&noexcept {
		static_assert(std::is_move_constructible<T>::value && std::is_convertible<U &&, T>::value,
		              "T must be move constructible and convertible from U");
		return has_value() ? **this : static_cast<T>(std::forward<U>(u));
	}
...
struct hash<duckdb::optional<T>> {
	::std::size_t operator()(const duckdb::optional<T> &o) const {
		if (!o.has_value()) {
			return 0;
		}

		return std::hash<duckdb::detail::remove_const_t<T>>()(*o);
	}
};
} // namespace std
