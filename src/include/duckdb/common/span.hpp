...
public:
	...
	reference front() const { D_ASSERT(!empty()); return *data(); }
	//! access the last element
	reference back() const { D_ASSERT(!empty()); return *(data() + (size() - 1)); }
	//! access specified element
	reference operator[](size_type idx) const { D_ASSERT(idx < size()); return *(data() + idx); }

	// Subviews
	//! obtains a subspan consisting of the first N elements of the sequence
	span first(size_type count_p) const { D_ASSERT(count_p <= size()); return { data(), count_p }; }
	//! obtains a subspan consisting of the last N elements of the sequence
	span last(size_type count_p) const { D_ASSERT(count_p <= size()); return { data() + (size() - count_p), count_p }; }
	//! obtains a subspan
	span subspan(size_type offset, size_type count_p) const { D_ASSERT(offset + count_p <= size()); return { data() + offset, count_p }; }
};
