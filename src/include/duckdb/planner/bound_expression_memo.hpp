...
//                         DuckDB
//
// duckdb/planner/bound_expression_memo.hpp
//
//
//===----------------------------------------------------------------------===//
...
//! BoundExpressionMemo maps parsed expression nodes (by identity) to their bound expressions.
...
private:
	...
	const MemoEntry &GetEntry(const ParsedExpression &node) const;
	void VerifyEntry(const ParsedExpression &node) const;

private:
	reference_map_t<const ParsedExpression, MemoEntry> entries;
...
//! RAII scope for BoundExpressionMemo entries: opened by every full bind cycle, erases the
//! entries inserted within it that were not consumed - also when unwinding on a binding error
class BoundExpressionScope {
public:
	explicit BoundExpressionScope(BoundExpressionMemo &memo);
	~BoundExpressionScope();

private:
	BoundExpressionMemo &memo;
};
