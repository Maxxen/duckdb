...
//!
//! Unlike ColumnList, this is purely a logical (declaration-order) list: it carries no
//! physical/storage mapping. The binder resolves a ParsedColumnList into a bound ColumnList,
//! at which point physical column indices and storage oids are assigned.
...
public:
	...
	DUCKDB_API const ParsedColumnDefinition &GetColumn(const string &name) const;
	...
	static ParsedColumnList Deserialize(Deserializer &deserializer);
...
private:
	void AddToNameMap(ParsedColumnDefinition &column, column_t oid);
