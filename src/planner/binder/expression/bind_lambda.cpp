...
BindResult ExpressionBinder::BindExpression(LambdaExpression &expr, idx_t depth,
	...
	for (idx_t i = 0; i < column_names.size(); i++) {
		column_types.push_back((*bind_lambda_function)(i, parameter_types[i]));
	}
...
void ExpressionBinder::TransformCapturedLambdaColumn(unique_ptr<Expression> &original,
                                                     ...
                                                     const vector<LogicalType> &lambda_parameter_types) {
	...
	if (original->GetExpressionClass() == ExpressionClass::BOUND_LAMBDA_REF) {
		...
		const auto lambda_param_index = bound_lambda_ref.binding.column_index;

		auto logical_type = (*bind_lambda_function)(lambda_param_index, lambda_parameter_types[lambda_param_index]);
...
void ExpressionBinder::CaptureLambdaColumns(BoundLambdaExpression &bound_lambda_expr, unique_ptr<Expression> &expr,
	...
	if (expr->GetExpressionClass() == ExpressionClass::BOUND_COLUMN_REF ||
		...
		ExpressionIterator::EnumerateChildren(*expr, [&](unique_ptr<Expression> &child) {
			CaptureLambdaColumns(bound_lambda_expr, child, bind_lambda_function, lambda_parameter_types);
		});
	}
