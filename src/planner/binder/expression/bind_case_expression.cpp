...
BindResult ExpressionBinder::BindExpression(CaseExpression &expr, idx_t depth) {
	...
	auto return_type = ExpressionBinder::GetExpressionReturnType(GetMemo().Get(*expr.ElseMutable()));
	for (auto &check : expr.CaseChecksMutable()) {
		auto then_type = ExpressionBinder::GetExpressionReturnType(GetMemo().Get(*check.then_expr));
	...
	for (auto &check : expr.CaseChecksMutable()) {
		auto when_expr = GetMemo().Consume(*check.when_expr);
		auto then_expr = GetMemo().Consume(*check.then_expr);
	...
	auto else_expr = GetMemo().Consume(*expr.ElseMutable());
