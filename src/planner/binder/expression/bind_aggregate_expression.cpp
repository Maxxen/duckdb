...
static void NegatePercentileFractions(ClientContext &context, BoundExpressionMemo &memo,
                                      unique_ptr<ParsedExpression> &fractions, bool desc) {
	D_ASSERT(fractions.get());
	D_ASSERT(memo.IsBound(*fractions));
	auto &bound = memo.GetMutable(*fractions);
...
BindResult BaseSelectBinder::BindAggregate(FunctionExpression &aggr, AggregateFunctionCatalogEntry &func, idx_t depth) {
	...
	for (idx_t i = 0; i < aggr.GetArguments().size(); ++i) {
		...
		if (!error.HasError() && ordered_set_agg && i == aggr.GetArguments().size() - 1) {
			NegatePercentileFractions(context, GetMemo(), child.GetExpressionMutable(), negate_fractions);
		}
	}
	...
	if (error.HasError()) {
		...
		if (bound_columns) {
			for (auto &child : aggr.GetArgumentsMutable()) {
				...
				ExtractCorrelatedExpressions(binder, GetMemo().Get(*child.GetExpressionMutable()));
			}
			...
			if (aggr.Filter()) {
				...
				ExtractCorrelatedExpressions(binder, GetMemo().Get(*aggr.Filter()));
			}
			if (aggr.OrderBy() && !aggr.OrderBy()->orders.empty()) {
				for (auto &order : aggr.OrderByMutable()->orders) {
					...
					ExtractCorrelatedExpressions(binder, GetMemo().Get(*order.expression));
				}
			}
		} else {
	...
	if (aggr.Filter()) {
		auto child = GetMemo().Consume(*aggr.Filter());
	...
	if (ordered_set_agg) {
		...
		if (aggr.GetArguments().size() < ordered_set_agg) {
			for (auto &order : aggr.OrderByMutable()->orders) {
				if (order_sensitive) {
					arguments.emplace_back(string(), GetMemo().Get(*order.expression).Copy());
				} else {
					arguments.emplace_back(string(), GetMemo().Consume(*order.expression));
				}
			}
		}
		if (!order_sensitive) {
	...
	if (!aggr.OrderBy()->orders.empty()) {
		...
		auto &config = DBConfig::GetConfig(context);
		for (auto &order : aggr.OrderByMutable()->orders) {
			auto order_expr = GetMemo().Consume(*order.expression);
