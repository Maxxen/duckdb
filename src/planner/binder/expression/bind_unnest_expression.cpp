...
BindResult UnnestBinder::Bind(FunctionExpression &function, idx_t depth, bool root_expression) {
	...
	if (args.size() != 1) {
		...
		for (idx_t i = 1; i < args.size(); i++) {
			...
			auto &const_child = binder.GetBoundExpressionMemo().Get(*args[i].GetExpressionMutable());
	...
	if (error.HasError()) {
		...
		ExpressionBinder::ExtractCorrelatedExpressions(binder,
		                                               binder.GetBoundExpressionMemo().Get(args[0].GetExpression()));
	}
	auto &child = binder.GetBoundExpressionMemo().GetMutable(*args[0].GetExpressionMutable());
