...
BindResult ExpressionBinder::BindExpression(OperatorExpression &op, idx_t depth) {
	...
	case ExpressionType::ARRAY_EXTRACT: {
		D_ASSERT(GetMemo().IsBound(*op.GetChildrenMutable()[0]));
		auto &b_exp = GetMemo().Get(*op.GetChildrenMutable()[0]);
		...
		if (b_exp_type.id() == LogicalTypeId::MAP) {
			...
			auto &i_exp = GetMemo().Get(*op.GetChildrenMutable()[1]);
	...
	case ExpressionType::STRUCT_EXTRACT: {
		...
		D_ASSERT(GetMemo().IsBound(*op.GetChildrenMutable()[0]));
		D_ASSERT(GetMemo().IsBound(*op.GetChildrenMutable()[1]));
		auto &extract_exp = GetMemo().Get(*op.GetChildrenMutable()[0]);
		if (extract_exp.HasParameter() || extract_exp.GetReturnType().id() == LogicalTypeId::UNKNOWN) {
			throw ParameterNotResolvedException();
		}
		auto &name_exp = GetMemo().Get(*op.GetChildrenMutable()[1]);
	...
	case ExpressionType::ARROW:
		...
		break;
	case ExpressionType::OPERATOR_TRY: {
		auto &expr = GetMemo().Get(*op.GetChildrenMutable()[0]);
		if (expr.HasSubquery()) {
	...
	if (!function_name.empty()) {
		...
		GetMemo().EraseSubtree(*function);
		return result;
	}

	vector<unique_ptr<Expression>> children;
	for (idx_t i = 0; i < op.GetChildrenMutable().size(); i++) {
		D_ASSERT(GetMemo().IsBound(*op.GetChildrenMutable()[i]));
		children.push_back(GetMemo().Consume(*op.GetChildrenMutable()[i]));
	}
