...
static unique_ptr<ParsedExpression> BindColumn(Binder &binder, ClientContext &context, const BindingAlias &alias,
	...
	// store the bound column in the memo, keyed by the parsed column reference placed in the join condition
	binder.GetBoundExpressionMemo().Insert(*expr, std::move(result));
	return expr;
}
...
BoundStatement Binder::Bind(JoinRef &ref) {
	...
	// scope for the bound USING/NATURAL join columns seeded into the memo by BindColumn
	BoundExpressionScope using_column_scope(GetBoundExpressionMemo());
