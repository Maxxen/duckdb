...
auto Convert(SOURCE) -> TARGET = delete;
...
inline auto Convert(duckdb_v2_identifier_t ident) -> std::string_view {
	return std::string_view(ident.ptr, ident.len);
}

inline auto Convert(const std::string &str) -> duckdb_v2_str {
	return duckdb_v2_str {str.data(), str.size()};
}
...
DUCKDB_V2_ERROR GetErrorCodeFromExceptionType(ExceptionType type);
optional<ExceptionType> GetExceptionTypeFromErrorCode(DUCKDB_V2_ERROR code);
...
struct CV2ErrorInfo {
	...
	[[noreturn]] void ThrowAsException() const {
		...
		if (const auto type = GetExceptionTypeFromErrorCode(code)) {
...
inline void BuildParameterMap(const duckdb_v2_identifier_t *parameter_names,
	...
	for (idx_t i = 0; i < parameter_count; i++) {
		...
		Identifier key = (name.ptr && name.len > 0) ? Identifier(Convert(name)) : Identifier(std::to_string(i + 1));
