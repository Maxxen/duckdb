...
DUCKDB_V2_ERROR duckdb_v2_value_to_string(duckdb_v2_value_handle value, char **out_string,
	...
	return WithErrorHandler(err, [&]() {
		...
		*out_string = nullptr;
		auto str = Convert(value)->ToString();
...
DUCKDB_V2_ERROR duckdb_v2_value_get_bignum(duckdb_v2_value_handle value, uint8_t **out_data, idx_t *out_length,
	...
	return WithErrorHandler(err, [&]() {
		if (!out_data || !out_length || !out_is_negative) {
			throw duckdb::InvalidInputException("null argument to duckdb_v2_value_get_bignum");
		}

		*out_data = nullptr;
		*out_length = 0;
		*out_is_negative = false;
		RequireTypedValue(value, duckdb::LogicalTypeId::BIGNUM, "duckdb_v2_value_get_bignum");

		// TODO: dont malloc...
		throw duckdb::NotImplementedException("duckdb_v2_value_get_bignum");

		/*
		auto &str = duckdb::StringValue::Get(*Convert(value));
