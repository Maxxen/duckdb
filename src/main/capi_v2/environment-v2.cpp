...
DUCKDB_V2_API_CALL_t duckdb_v2_create_environment(duckdb_v2_environment_handle *out_env,
...
DUCKDB_V2_API_CALL_t duckdb_v2_destroy_environment(duckdb_v2_environment_handle *env) {
	if (!env || !*env) {
...
DUCKDB_V2_API_CALL_t duckdb_v2_environment_database_count(duckdb_v2_environment_handle env, idx_t *out_count,
                                                          duckdb_v2_error_info_handle *err) {
	return duckdb::WithErrorHandler(err, [&]() {
		if (!env || !out_count) {
