...
DUCKDB_V2_ERROR duckdb_v2_custom_type_builder_create(duckdb_v2_custom_type_builder_handle *out_builder,
                                                     duckdb_v2_error_info_handle *err) {
	return duckdb::WithErrorHandler(err, [&]() {
		if (!out_builder) {
...
DUCKDB_V2_ERROR duckdb_v2_custom_type_builder_register_with_connection(duckdb_v2_connection_handle conn,
	...
	return duckdb::WithErrorHandler(err, [&]() {
		if (!conn) {
			throw duckdb::InvalidInputException("null connection argument to duckdb_v2_custom_type_builder_register");
		}
		if (!builder) {
		...
		ctx.RunFunctionInTransaction([&]() { RegisterCustomTypeV2(ctx, b); });
	});
}
