...
vector<string> GeometryTypeSet::Format(bool geoparquet_case) const {
	...
	for (idx_t i = 0; i < 4; i++) {
		...
		Scan([&](GeometryType gtype) {
			const auto vtype = gtype.GetVertType();
			const auto ptype = gtype.GetPartType();
...
string GeometryStats::ToString(const BaseStatistics &stats) {
	...
	const auto extent = StringUtil::Format("[XMin: %g, XMax: %g, YMin: %g, YMax: %g]", geometry_data.bbox.xmin,
	                                       geometry_data.bbox.xmax, geometry_data.bbox.ymin, geometry_data.bbox.ymax);
...
void GeometryStats::Serialize(const BaseStatistics &stats, Serializer &serializer) {
	...
	serializer.WriteProperty(200, "xmin", geometry_data.bbox.xmin);
	serializer.WriteProperty(201, "xmax", geometry_data.bbox.xmax);
	serializer.WriteProperty(202, "ymin", geometry_data.bbox.ymin);
	serializer.WriteProperty(203, "ymax", geometry_data.bbox.ymax);

	serializer.WritePropertyWithDefault<uint8_t>(210, "xy_bitset", geometry_data.types.bits[0]);
	serializer.WritePropertyWithDefault<uint8_t>(211, "xyz_bitset", geometry_data.types.bits[1]);
	serializer.WritePropertyWithDefault<uint8_t>(212, "xym_bitset", geometry_data.types.bits[2]);
	serializer.WritePropertyWithDefault<uint8_t>(213, "xyzm_bitset", geometry_data.types.bits[3]);
}

void GeometryStats::Deserialize(Deserializer &deserializer, BaseStatistics &stats) {
	auto &geometry_data = GeometryStats::GetDataUnsafe(stats);
	geometry_data.bbox.xmin = deserializer.ReadProperty<double>(200, "xmin");
	geometry_data.bbox.xmax = deserializer.ReadProperty<double>(201, "xmax");
	geometry_data.bbox.ymin = deserializer.ReadProperty<double>(202, "ymin");
	geometry_data.bbox.ymax = deserializer.ReadProperty<double>(203, "ymax");

	geometry_data.types.bits[0] = deserializer.ReadPropertyWithDefault<uint8_t>(210, "xy_bitset");
	geometry_data.types.bits[1] = deserializer.ReadPropertyWithDefault<uint8_t>(211, "xyz_bitset");
	geometry_data.types.bits[2] = deserializer.ReadPropertyWithDefault<uint8_t>(212, "xym_bitset");
	geometry_data.types.bits[3] = deserializer.ReadPropertyWithDefault<uint8_t>(213, "xyzm_bitset");
}
...
void GeometryStats::Update(BaseStatistics &stats, const string_t &value) {
	...
	if (Geometry::GetExtent(value, bounds) != 0) {
		geometry_data.bbox.Merge(bounds);
	}

	// Add geometry and vertex type to the statistics
	const auto type = Geometry::GetType(value);
...
void GeometryStats::Merge(BaseStatistics &stats, const BaseStatistics &other) {
	...
	geometry_data.bbox.Merge(other_geometry_data.bbox);
