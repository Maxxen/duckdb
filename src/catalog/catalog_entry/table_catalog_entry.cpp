...
//! The bound columns know their own logical index; parsed columns are in declaration order
static LogicalIndex ColumnLogicalIndex(const ColumnDefinition &column, idx_t pos) {
	return column.Logical();
}

static LogicalIndex ColumnLogicalIndex(const ParsedColumnDefinition &column, idx_t pos) {
	return LogicalIndex(pos);
}
...
static string ColumnsToSQLInternal(const COLUMN_LIST &columns, const vector<unique_ptr<Constraint>> &constraints) {
	...
	for (auto &column : columns.Logical()) {
		auto logical = ColumnLogicalIndex(column, pos++);
		if (logical.index > 0) {
			ss << ", ";
		}
...
static string ColumnNamesToSQLInternal(const COLUMN_LIST &columns) {
	...
	idx_t pos = 0;
	for (auto &column : columns.Logical()) {
		if (ColumnLogicalIndex(column, pos++).index > 0) {
			ss << ", ";
		}
