...
void QueryTableFunction::RegisterFunction(BuiltinFunctions &set) {
	...
	query_table_function.GetSignature() = FunctionSignature({LogicalType::LIST(LogicalType::VARCHAR)});
	query_table.AddFunction(query_table_function);
	// add by_name option
	query_table_function.GetSignature().AddParemeter("", LogicalType::BOOLEAN);
	query_table.AddFunction(query_table_function);
