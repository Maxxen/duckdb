...
static void ArrowToDuckDBArray(Vector &vector, ArrowArray &array, ArrowArrayScanState &array_state, idx_t size,
	...
	auto child_offset = (scan_state.chunk_offset + array.offset) * array_size;
	if (nested_offset != -1) {
		child_offset = array_size * nested_offset;
	}

	SetValidityMask(vector, array, scan_state, size, nested_offset);

	auto &child_vector = ArrayVector::GetEntry(vector);
	SetValidityMask(child_vector, *array.children[0], scan_state, child_count, child_offset);
