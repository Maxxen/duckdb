...
static unique_ptr<FunctionData> CreateExternalResourceBind(ClientContext &context, TableFunctionBindInput &input,
	...
	for (auto &np : input.named_parameters) {
		...
		if (key == "params" && !np.second.IsNull()) {
			...
			// accept a struct literal as well as a map, e.g. handle := {'id': 'i-123'}
			result->adopt_handle =
...
void CreateExternalResourceFun::RegisterFunction(BuiltinFunctions &set) {
	...
	fn.AddNamedParameter("params", LogicalType::MAP(LogicalType::VARCHAR, LogicalType::VARCHAR));
	fn.AddNamedParameter("resource_name", LogicalType::VARCHAR);
	fn.AddNamedParameter("handle", LogicalType::ANY);
	fn.AddNamedParameter("teardown_on_failure", LogicalType::BOOLEAN);
	fn.AddNamedParameter("timeout_seconds", LogicalType::BIGINT);
	fn.AddNamedParameter("poll_interval_seconds", LogicalType::BIGINT);
	set.AddFunction(fn);
}
