...
TableFunction::TableFunction(Identifier name, const vector<LogicalType> &arguments, table_function_t function_,
    ...
    : SimpleFunction(std::move(name), arguments, LogicalType(LogicalTypeId::INVALID)), bind(bind),
      bind_replace(nullptr), bind_operator(nullptr), init_global(init_global), init_local(init_local),
...
string TableFunction::ToString() const {
	return Function::CallToString(GetCatalogName(), GetSchemaName(), name, GetArgumentTypes(), GetNamedParameters());
}

bool TableFunction::operator==(const TableFunction &rhs) const {
	return name == rhs.name && signature.Equal(rhs.GetSignature()) && bind == rhs.bind &&
...
bool TableFunction::Equal(const TableFunction &rhs) const {
	// compares the argument types and the varargs - deliberately not the parameter names
	return signature.Equal(rhs.GetSignature());
}
