...
static void UnionTagFunction(DataChunk &args, ExpressionState &state, Vector &result) {
    ...
    for (idx_t tag_idx = 0; tag_idx < types.size(); tag_idx++) {
        ...
        tag_name_vector_entries[tag_idx] = tag_str.size() < 12 
            ? string_t(tag_str) 
...
ScalarFunction UnionTagFun::GetFunction() {
	...
	return ScalarFunction("union_tag", {LogicalTypeId::UNION}, LogicalType::VARCHAR,
	                      UnionTagFunction, UnionTagBind, nullptr, nullptr); // TODO: Statistics?
}
