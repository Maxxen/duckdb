...
static void UnionExtractFunction(DataChunk &args, ExpressionState &state, Vector &result) {
	...
	auto &children = UnionVector::GetEntries(vec);
	D_ASSERT(info.index < children.size());
	auto &union_child = children[info.index];
	result.Reference(*union_child);
...
static unique_ptr<BaseStatistics> PropagateUnionExtractStats(ClientContext &context, FunctionStatisticsInput &input) {
	auto &child_stats = input.child_stats;
	auto &bind_data = input.bind_data;
	if (!child_stats[0]) {
		return nullptr;
	}
	auto &union_stats = (UnionStatistics &)*child_stats[0];
	auto &info = (UnionExtractBindData &)*bind_data;
	if (info.index >= union_stats.child_stats.size() || !union_stats.child_stats[info.index]) {
		return nullptr;
	}
