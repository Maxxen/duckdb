...
ArrowSchemaMetadata::ArrowSchemaMetadata(const char *metadata) {
	...
	extension_metadata_map = JsonValue::TryParse(schema_metadata_map[ARROW_METADATA_KEY], ignore_errors);
}

ArrowSchemaMetadata::ArrowSchemaMetadata() {
	// Always initialize out metadata map
	extension_metadata_map = JsonValue(JsonKind::OBJECT);
}
...
ArrowSchemaMetadata ArrowSchemaMetadata::NonCanonicalType(const string &type_name, const string &vendor_name) {
	...
	metadata.AddOption(ARROW_METADATA_KEY, metadata.extension_metadata_map.ToString());
	return metadata;
}
...
ArrowExtensionMetadata ArrowSchemaMetadata::GetExtensionInfo(string format) {
	return {schema_metadata_map[ARROW_EXTENSION_NAME], extension_metadata_map["vendor_name"].AsString(),
	        extension_metadata_map["type_name"].AsString(), std::move(format)};
}
