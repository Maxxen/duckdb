...
public:
	...
	double MatchNumber() {
		...
		while (pos < end &&
		       (isdigit(*pos) || *pos == '.' || *pos == '-' || *pos == '+' || *pos == 'e' || *pos == 'E')) {
			pos++;
		}
		if (start == pos) {
...
static void FromStringRecursive(TextReader &reader, BinaryWriter &writer, idx_t depth, bool parent_has_z,
                                bool parent_has_m) {
...
static void ToStringRecursive(BinaryReader &reader, TextWriter &writer, idx_t depth, bool parent_has_z,
                              bool parent_has_m) {
	if (depth == Geometry::MAX_RECURSION_DEPTH) {
...
pair<GeometryType, VertexType> Geometry::GetGeometryType(const string_t &geom) {
	BinaryReader reader(geom.GetData(), geom.GetSize());
	const auto byte_order = reader.Read<uint8_t>();
	if (byte_order != 1) {
		throw InvalidInputException("Unsupported byte order %d in WKB", byte_order);
	...
	const auto meta = reader.Read<uint32_t>();
	const auto type = static_cast<GeometryType>(meta % 1000);
	const auto vert = static_cast<VertexType>(meta / 1000);
...
static GeometryVerificationResult VerfiyRecursive(BinaryReader &reader, uint32_t depth, uint32_t parent_type, uint32_t
parent_flag) { if (depth > Geometry::MAX_RECURSION_DEPTH) {
