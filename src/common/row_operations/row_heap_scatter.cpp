...
ArrayParentValidity::ArrayParentValidity(data_ptr_t validitymask_location)
    : validitymask_location(validitymask_location) {
}

void ArrayParentValidity::SetInvalid(idx_t idx) {
	// TODO: we probably need to increment the pointer if the array size > 64?
	// TODO: Also: how does "chunk size" affect this?
	const auto bit = ~(1UL << idx);
	*validitymask_location &= bit;
}

bool ArrayParentValidity::IsValid(idx_t idx) {
	ValidityBytes row_mask(validitymask_location);
	const auto valid = row_mask.RowIsValid(row_mask.GetValidityEntry(0), idx);
	return valid;
}

StructParentValidity::StructParentValidity(data_ptr_t *validitymask_locations, idx_t child_vector_index)
...
void StructParentValidity::SetInvalid(idx_t idx) {
	const auto bit = ~(1UL << idx_in_entry);
	*(validitymask_locations[idx] + entry_idx) &= bit;
}

bool StructParentValidity::IsValid(idx_t idx) {
...
static void TemplatedHeapScatter(UnifiedVectorFormat &vdata, const SelectionVector &sel, idx_t count,
                                 data_ptr_t *key_locations, ParentValidity *parent_validity, idx_t offset) {
...
static void HeapScatterStructVector(Vector &v, idx_t vcount, const SelectionVector &sel, idx_t ser_count,
	...
	for (idx_t i = 0; i < children.size(); i++) {
		...
		StructParentValidity struct_validity(struct_validitymask_locations, i);
...
static void HeapScatterArrayVector(Vector &v, idx_t vcount, const SelectionVector &sel, idx_t ser_count,
	...
	for (idx_t i = 0; i < ser_count; i++) {
		...
		while (elem_remaining > 0) {
			...
			// Temp validity ptr
			// data_ptr_t temp_pointers[STANDARD_VECTOR_SIZE];
			// for (idx_t elem_idx = 0; elem_idx < chunk_size; elem_idx++) {
			//	temp_pointers[elem_idx] = array_validitymask_location; //+ (elem_idx / 8); // Or add this?
			//}

			ArrayParentValidity array_validity(array_validitymask_location);
