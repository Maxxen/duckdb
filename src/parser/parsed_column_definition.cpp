...
ParsedColumnDefinition::ParsedColumnDefinition(string name_p, unique_ptr<ParsedExpression> type_expression_p)
    : name(std::move(name_p)), type_expression(std::move(type_expression_p)) {
}

ParsedColumnDefinition::ParsedColumnDefinition(string name_p, unique_ptr<ParsedExpression> type_expression_p,
                                               unique_ptr<ParsedExpression> expression_p, TableColumnType category_p)
...
ParsedColumnDefinition ParsedColumnDefinition::Copy() const {
	ParsedColumnDefinition copy(name, type_expression ? type_expression->Copy() : nullptr,
	                            expression ? expression->Copy() : nullptr, category);
...
const string &ParsedColumnDefinition::Name() const {
	return name;
}
...
const ParsedExpression &ParsedColumnDefinition::Type() const {
	if (!type_expression) {
		throw InternalException("Type() called on a ParsedColumnDefinition without a type");
	}
...
static void InnerGetListOfDependencies(const ParsedExpression &expr, vector<string> &dependencies) {
	if (expr.GetExpressionType() == ExpressionType::COLUMN_REF) {
		...
		dependencies.push_back(columnref.GetColumnName());
	}
